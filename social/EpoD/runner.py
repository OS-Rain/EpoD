...
class Runner(object):
    ...
    def train(self, epoch, data):
        ...
        embeddings, cs, ss, pred_x_list, kl_loss = self.model([
        ...
        for t in range(self.len_train - 1):
            ...
            pred_x_pos_edge_index = prepare(data, t)[0]
            if args.dataset == 'yelp':
                neg_edge_index = bi_negative_sampling(pos_edge_index,
                                                      args.num_nodes,
                                                      args.shift)
                pred_x_neg_edge_index = bi_negative_sampling(pred_x_pos_edge_index,
                                                      args.num_nodes,
                                                      args.shift)
            else:
                neg_edge_index = negative_sampling(
                    ...
                    args.sampling_times)
                pred_x_neg_edge_index = negative_sampling(
                    pred_x_pos_edge_index,
                    num_neg_samples=pred_x_pos_edge_index.size(1) *
                    args.sampling_times)
            edge_index.append(
                torch.cat([pos_edge_index, neg_edge_index], dim=-1))
            
            x_edge_index.append(
                torch.cat([pred_x_pos_edge_index, pred_x_neg_edge_index], dim=-1))
            pos_y = z.new_ones(pos_edge_index.size(1)).to(device)
            neg_y = z.new_zeros(neg_edge_index.size(1)).to(device)
            pred_x_pos_y = z.new_ones(pred_x_pos_edge_index.size(1)).to(device)
            pred_x_neg_y = z.new_zeros(pred_x_neg_edge_index.size(1)).to(device)
            edge_label.append(torch.cat([pos_y, neg_y], dim=0))
            tsize.append(pos_edge_index.shape[1] * 2)
            pred_x_edge_label.append(torch.cat([pred_x_pos_y, pred_x_neg_y], dim=0))

        edge_label = torch.cat(edge_label, dim=0)
        pred_x_edge_label = torch.cat(pred_x_edge_label, dim=0)
        ...
        def cal_x(embeddings, decoder):
            preds = torch.tensor([]).to(device)
            for t in range(self.len_train - 1):
                z = embeddings[t]
                pred = decoder(z, x_edge_index[t])
                preds = torch.cat([preds, pred])
            return preds
        ...
        cx = cal_x(pred_x_list, self.model.cs_decoder)

        conf_loss = cal_loss(sy, edge_label)
        causal_loss = cal_loss(cy, edge_label)
        pred_x_causal_loss = cal_loss(cx, pred_x_edge_label)
        ...
        # loss = causal_loss + alpha * pred_x_causal_loss 
        # loss = causal_loss + alpha * pred_x_causal_loss + beta * kl_loss
        # loss = causal_loss 
        # loss = causal_loss + beta *  kl_loss
        # loss = causal_loss + la * penalty + alpha *  pred_x_causal_loss + beta *  torch.exp(-kl_loss)
        # loss = causal_loss +0.1 * pred_x_causal_loss + 0.1 *  torch.exp(-kl_loss)
        # loss = causal_loss  +  pred_x_causal_loss + beta * torch.exp(-kl_loss)
        # loss = causal_loss + la * penalty + beta * torch.exp(-kl_loss)
        # loss = causal_loss + la * penalty + pred_x_causal_loss + beta * torch.exp(-kl_loss)
        loss = causal_loss + la * penalty + pred_x_causal_loss + beta * torch.exp(-kl_loss)
        # loss = causal_loss + pred_x_causal_loss + beta * torch.exp(-kl_loss)
        # loss = causal_loss +  pred_x_causal_loss + beta * torch.exp(-kl_loss)
        # loss = causal_loss + la * penalty +  pred_x_causal_loss - kl_loss
        # loss = causal_loss + la * penalty +  pred_x_causal_loss + beta * torch.exp(-kl_loss)
        # loss = causal_loss + la * penalty 
        # loss = causal_loss + la * penalty + pred_x_causal_loss +  torch.exp(-kl_loss)
        # loss = causal_loss  + pred_x_causal_loss + beta *  torch.exp(-kl_loss)
        # loss = causal_loss  + alpha *  pred_x_causal_loss - beta *  kl_loss
        ...
        with tqdm(range(1, args.max_epoch + 1)) as bar:
            for epoch in bar:
                ...
                if epoch == 1 or epoch % self.args.log_interval == 0:
                    cur_test_results = self.test(epoch, self.data['test'])
                    ...
                    print(
                        f"Current Test: Epoch:{cur_test_results[0]}, Train AUC:{cur_test_results[1]:.4f}, Val AUC: {cur_test_results[2]:.4f}, Test AUC: {cur_test_results[3]:.4f}"
                    )
