...
class EpoD(nn.Module):

    def __init__(self, args=None):
        super(EpoD, self).__init__()
        self.args = args
        ...
        self.context_init_prompt = nn.Parameter(torch.ones(1, in_dim), requires_grad=True)
        self.transformer_decoder_layer = nn.TransformerDecoderLayer(d_model=in_dim, nhead=2, dim_feedforward=8, dropout=0.2)# 32
        self.transformer_decoder = nn.TransformerDecoder(self.transformer_decoder_layer, num_layers=1)
        self.out_fc = nn.Linear(in_dim, hid_dim)
        ...
        # self.linear = nn.Linear(2*in_dim, hid_dim, bias=bool(args.lin_bias))
        self.linear = nn.Linear(2*in_dim, hid_dim, bias=bool(args.lin_bias))
        self.prompt_linear = nn.Linear(in_dim, hid_dim, bias=bool(args.lin_bias))
        self.layers = nn.ModuleList(DGNNLayer(hid_dim, hid_dim, n_heads=n_heads, norm=norm, dropout=args.dropout, skip=args.skip, use_RTE=args.use_RTE, sample_r=0.1, use_fmask=args.fmask, only_causal=args.only_causal) for i in range(self.n_layers))
        self.act = F.relu
    ...
    def reset_parameter(self):
        ...
        # nn.init.xavier_normal_(self.context_init_prompt)
        glorot(self.feat)
 
    def forward(self, edge_index_list, x_list):
        x_tensor = torch.stack(x_list)
        # prompt_answers = self.transformer_decoder(self.context_init_prompt.unsqueeze(0).expand(len(x_list), -1, -1).repeat(1,5,1), x_tensor)
        prompt_answers = self.transformer_decoder(self.context_init_prompt.unsqueeze(0).expand(len(x_list), self.num_nodes, -1), x_tensor)
        prompt_answers += torch.empty_like(prompt_answers).uniform_(1e-10, 1 - 1e-10)
        # kl_loss = torch.kl_div(torch.sigmoid(prompt_answers), torch.sigmoid(x_tensor)).mean()
        # kl_loss = torch.kl_div(torch.softmax(prompt_answers, dim=2), torch.softmax(x_tensor, dim=2)).mean()
        kl_loss = torch.kl_div(x_tensor.softmax(dim=-1).log(),prompt_answers.softmax(dim=-1)).sum()
        pred_x_list = [self.act(self.out_fc(prompt_answer)) for prompt_answer in prompt_answers]
        x_list = [self.act(self.linear(torch.cat([x_list[i],prompt_answers[i]], dim=1)))for i in range(len(x_list))]
        # x_list = [torch.cat([x_list[i],prompt_answers[i]], dim=1) for i in range(len(x_list))]

        for i, layer in enumerate(self.layers):
            x_list, cs, ss = layer(edge_index_list, x_list)
            # _, cs_x, _ = layer(edge_index_list, prompt_answers_list)
            if i != len(self.layers)-1:
                x_list = [self.act(x) for x in x_list]

        return x_list, cs, ss, pred_x_list, kl_loss

    def get_subgraph(self, edge_index_list, prompt_answers):
        for i, edges  in enumerate(edge_index_list):
            ...
            row = edges[0]
            col = edges[1]
            score = F.softmax(-torch.cdist(prompt_answers[i][row], prompt_answers[i][col], p=1), dim=1).sigmoid()[0]
            sample = torch.bernoulli(score)
            edge_index_list[i] = torch.stack([row[sample!=0], col[sample!=0]])
